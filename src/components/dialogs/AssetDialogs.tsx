...
function NoMap({ entry, title, icon }: { entry: DialogProps["entry"]; title: string; icon: React.ReactNode }) {
  return <DialogFrame dialogKey={entry.key} title={title} icon={icon} size="sm"><p className="hint">{t("Open or create a map first.")}</p></DialogFrame>;
}
...
export function StringEditorDialog({ entry }: DialogProps) {
  ...
  if (!scenario || !list) return <NoMap entry={entry} title={t("String Editor")} icon={<Type size={14} />} />;
  ...
  return (
    ...
    >
      <div className="row">
        ...
        <TextInput placeholder={t("Search text, usages or #…")} value={q} onChange={(e) => setQ(e.target.value)} />
        <Button size="sm" onClick={addString} title={t("Append an empty string at the end of the table")}><Plus size={12} /> {" "}{t("Add string")}</Button>
        <Button size="sm" disabled={unused + empty === 0} onClick={() => setList(deleteUnused(list, usages))} title={t("Blank every string nothing refers to (slots keep their numbers)")}><Trash2 size={12} /> {" "}{t("Delete unused")}</Button>
      </div>
      <div className="split rows" style={{ ["--split" as string]: "1fr" }}>
        ...
        <div className="col" style={{ gap: 6 }}>
          <span className="dim" style={{ fontSize: 11 }}>
            {t("String #")}{current === undefined ? "—" : sel} · {(usages.get(sel) ?? []).map((u) => u.label).join(", ") || (list[sel] === null ? t("blank slot") : t("not referenced"))}
          </span>
          <ColorTextField
            value={current ?? ""}
            ...
            placeholder={sel > 0 && sel < list.length ? t("Empty string") : t("Select a string to edit it")}
          />
          <div className="row between">
            <p className="hint" style={{ margin: 0 }}>{t("Bytes below 0x20 are shown as <XX> and may be typed that way; tab and line breaks stay literal.")}</p>
            <Check
              label={t("1.16.1 colours")}
              title={t("Reset the colour at every line break, the way 1.16.1 drew it. Remastered carries a colour onto the next line instead — if the string changes when you tick this, it renders differently now than when it was written. The setting is the editor's, so every preview in it follows.")}
...
const KEEP = "keep";
const IMPORT_AS = [...WAV_PRESETS.map((p) => ({ value: p.id, label: p.label })), { value: KEEP, label: msg("Keep PCM WAV and Ogg files as they are") }];
...
function formatOf(bytes: Uint8Array | undefined, path: string): { label: string; wav: WavInfo | null } {
  ...
  return { label: ext === "ogg" || ext === "oga" ? t("Ogg") : ext ? ext.toUpperCase() : "?", wav: null };
}
...
export function SoundEditorDialog({ entry }: DialogProps) {
  ...
  if (!scenario || !form) return <NoMap entry={entry} title={t("Sound Editor")} icon={<Music size={14} />} />;
  ...
  const prepare = async (name: string, bytes: Uint8Array): Promise<{ bytes: Uint8Array; name: string; note: SoundNote }> => {
    ...
    if (!target && ((wav && isPlainPcm(wav)) || isOgg)) {
      return { bytes, name, note: { name, level: "ok", text: wav ? t("kept as it is ({wavFormatLabel})", { wavFormatLabel: wavFormatLabel(wav) }) : t("kept as it is (Ogg plays in Remastered only)") } };
    }
    const want = target ?? DEFAULT_WAV_PRESET.target;
    const result = await convertToWav(bytes, want);
    const label = wavFormatLabel(parseWavHeader(result.bytes)!);
    if (!result.converted) return { bytes, name, note: { name, level: "ok", text: t("already {label}", { label }) } };
    const out = withWavExtension(name);
    const from = wav ? wavFormatLabel(wav) : (name.match(/\.([^.\\/]+)$/)?.[1] ?? "file").toUpperCase();
    return { bytes: result.bytes, name: out, note: { name, level: "ok", text: `${from} → ${label}, ${mmss(result.seconds)}, ${kb(result.bytes.length)}${out !== name ? t(", stored as {file}", { file: out.split(/[\\/]/).pop() ?? out }) : ""}` } };
  };
  ...
  const importFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        setBusy(t("Converting {name}…", { name: file.name }));
        try {
          ...
          done.push({ name: file.name, level: "warn", text: t("not imported: the {noun} could not decode it{reason}", { noun: hostTerms().noun, reason: err instanceof Error && err.message ? ` (${err.message})` : "" }) });
        }
      }
    } finally {
      setBusy(null);
  ...
  const convertRow = async (r: SoundRow) => {
    ...
    setBusy(t("Converting {pop}…", { pop: r.path.split(/[\\/]/).pop() ?? "" }));
    try {
      ...
      setNotes([{ name: r.path, level: "warn", text: t("not converted: the {noun} could not decode it{reason}", { noun: hostTerms().noun, reason: err instanceof Error && err.message ? ` (${err.message})` : "" }) }]);
    } finally {
      setBusy(null);
    }
  };
  ...
  const length = (r: SoundRow) => {
    ...
    return d === null ? <span className="faint" title={t("Neither the editor's WAV decoder nor the {noun} could read this file", { noun: hostTerms().noun })}>{t("cannot decode")}</span> : mmss(d);
  };
  ...
  const format = (r: SoundRow) => {
    ...
    return <span className={odd ? "warn" : "dim"} title={odd ? (f.wav && canDecodeWav(f.wav) ? t("Not plain PCM: the game plays its own ADPCM, other encodings it may not — Convert re-encodes it as PCM") : t("Not plain PCM: the game may not play it, and this editor cannot convert it")) : undefined}>{f.label}</span>;
  };

  return (
    <DialogFrame
      dialogKey={entry.key}
      title={t("Sound Editor")}
      icon={<Music size={14} />}
      size="lg"
      tall
      showApply
    ...
    >
      ...
      <div className="row">
        <Button size="sm" onClick={() => fileRef.current?.click()} disabled={filled >= WAV_SLOTS || !!busy}><Upload size={12} /> {" "}{t("Import…")}</Button>
        <label className="row" style={{ gap: 6 }}>
          <span className="dim" style={{ fontSize: 11 }}>as</span>
          <Select value={preset} options={IMPORT_AS} onChange={(e) => setPreset(e.target.value)} disabled={!!busy} style={{ minWidth: 290 }} title={t("Files are decoded by the {noun} and written as PCM WAV in this format. The game's own sounds are 22050 Hz, 16-bit, mono.", { noun: hostTerms().noun })} />
        </label>
        <Button size="sm" disabled={!canConvert} onClick={() => row && void convertRow(row)} title={target ? t("Re-encode the selected .wav to the chosen format") : t("Choose a format to convert to")}><RefreshCw size={12} /> {" "}{t("Convert")}</Button>
        {playing && row?.member === playing
          ? <Button size="sm" onClick={stop}><Square size={12} /> {" "}{t("Stop")}</Button>
          : <Button size="sm" disabled={!row?.member || !rowDecodes} onClick={() => row && void play(row)}><Play size={12} /> {" "}{t("Play")}</Button>}
        <Button size="sm" disabled={!row || !!busy} onClick={() => row && remove(row)}><Trash2 size={12} /> {" "}{t("Remove")}</Button>
        <span className="grow" />
        <span className="hint">{busy ?? <>{t("Files go in")}{" "}<span className="mono">{t("staredit\\wav\\")}</span> {" "}{t("inside the .scx")}</>}</span>
      </div>
      {notes.length > 0 && (
      ...
      <div className="listbox" style={{ flex: 1, minHeight: 200 }}>
        <table className="table">
          <thead><tr><th style={{ width: 40 }}>#</th><th>{t("Path in archive")}</th><th style={{ width: 70 }}>{t("Size")}</th><th style={{ width: 170 }}>{t("Format")}</th><th style={{ width: 70 }}>{t("Length")}</th><th style={{ width: 150 }}>{t("Used by")}</th></tr></thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.slot} className={sel === r.slot ? "selected" : ""} onClick={() => setSel(r.slot)} onDoubleClick={() => void play(r)}>
                <td className="num">{r.slot}</td>
                <td className="mono">{r.path}{!r.present && <span className="badge warn" style={{ marginLeft: 6 }} title={t("The table lists this path but the archive has no such file")}>missing</span>}{playing === r.member && <span className="badge teal" style={{ marginLeft: 6 }}>playing</span>}</td>
                <td className="num">{r.present ? kb(r.size) : "—"}</td>
                <td>{format(r)}</td>
                <td className="num">{length(r)}</td>
                <td className={r.usedBy.length ? "dim" : "faint"} title={r.usedBy.join(", ")}>{r.usedBy.length ? r.usedBy.join(", ") : t("not referenced")}</td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={6} className="hint">{t("No sounds in the table — import a file, or add one of the archive's files below.")}</td></tr>}
          </tbody>
        </table>
      </div>
      {orphans.length > 0 && (
        <div className="col" style={{ gap: 4 }}>
          <span className="dim" style={{ fontSize: 11 }}>{t("In the archive but not in the table:")}</span>
          <div className="row wrap" style={{ gap: 4 }}>
            {orphans.map((name) => <Button key={name} size="sm" onClick={() => addOrphan(name)} title={t("Add {name} to the WAV table", { name })}><Plus size={11} /> <span className="mono">{name}</span></Button>)}
          </div>
        </div>
      )}
      <p className="hint">{t("Import takes MP3, FLAC, AAC, Ogg and WAV (anything the {noun} decodes) and writes PCM WAV in the chosen format, which every StarCraft build plays; keep an Ogg only for Remastered. Remove clears the slot; the file is dropped from the archive too unless another slot or a trigger still refers to it. Triggers play a sound by its string, so a removed slot leaves the string in the table until String Editor's Delete unused.", { noun: hostTerms().noun })}</p>
    </DialogFrame>
  );
}
/* ── Switches ───────────────────────────────────────────── */
...
export function SwitchesDialog({ entry }: DialogProps) {
  ...
  if (!scenario || !names) return <NoMap entry={entry} title={t("Switches")} icon={<ToggleLeft size={14} />} />;
      ...
      <div className="split rows" style={{ ["--split" as string]: "1fr" }}>
        <ListBox
          ...
          render={(n, i) => (
            <>
              ...
              {n ? <span>{n}</span> : <span className="faint">{t("Switch {v}", { v: i + 1 })}</span>}
              <span className="row" style={{ marginLeft: "auto", gap: 4 }}>
                {usage[i] > 0 && <span className="dim" style={{ fontSize: 10 }} title={t("Switch conditions and Set Switch actions")}>{usage[i]} use{usage[i] === 1 ? "" : "s"}</span>}
                {n && <span className="badge teal">{t("named")}</span>}
              </span>
            </>
          )}
        />
        <div className="col" style={{ gap: 8, flex: "none" }}>
          <div className="form wide">
            <label>{t("Switch {v}", { v: sel + 1 })}</label>
            <div className="row">
              <TextInput key={sel} placeholder={t("Switch {v}", { v: sel + 1 })} value={names[sel]} onChange={(e) => rename(sel, e.target.value)} maxLength={255} />
              <Button size="sm" disabled={names[sel] === ""} onClick={() => rename(sel, "")} title={t("Back to the default name")}><Eraser size={12} /> {" "}{t("Clear")}</Button>
            </div>
            <label>{t("Used by")}</label>
            <span className="dim">{usage[sel] > 0 ? `${usage[sel]} condition${usage[sel] === 1 ? "" : "s"} / action${usage[sel] === 1 ? "" : "s"}` : t("not referenced by any trigger")}</span>
          </div>
          <p className="hint">{t("Names are stored in the string table (SWNM); an identical string is reused, a cleared name goes back to \"Switch N\". Text triggers and the script use the name.")}</p>
        </div>
      </div>
    </DialogFrame>
  );
...
export function LocationListDialog({ entry }: DialogProps) {
  ...
  return (
    <DialogFrame
      ...
      footerLeft={<span>{t("{length} / {capacity} locations · slot 63 is Anywhere · double-click to go to one", { length: locations.length, capacity })}</span>}
    >
      <div className="row">
        <Button size="sm" disabled={!scenario} onClick={() => tools.createInView()}><Plus size={12} /> {" "}{t("New")}</Button>
        <Button size="sm" disabled={first === undefined} onClick={() => open("locationProperties", { index: first })}><Pencil size={12} /> {" "}{t("Properties…")}</Button>
        <Button size="sm" disabled={!selected.some((i) => i !== ANYWHERE_INDEX)} onClick={() => tools.deleteSelected()}><Trash2 size={12} /> {" "}{t("Delete")}</Button>
        <span className="grow" />
        <Select style={{ width: 160 }} value={sort} onChange={(e) => setSort(e.target.value as typeof sort)} options={[{ value: "slot", label: t("Sort by slot") }, { value: "name", label: t("Sort by name") }, { value: "size", label: t("Sort by size") }]} />
      </div>
      <div className="listbox grow">
        <table className="table">
          <thead>
            <tr><th style={{ width: 40 }}>{t("Slot")}</th><th>{t("Name")}</th><th style={{ width: 66 }}>{t("Left")}</th><th style={{ width: 66 }}>{t("Top")}</th><th style={{ width: 66 }}>{t("Right")}</th><th style={{ width: 66 }}>{t("Bottom")}</th><th style={{ width: 90 }}>{t("Tiles")}</th><th style={{ width: 60 }} title={t("Elevations the location applies on")}>{t("Elev.")}</th></tr>
          </thead>
          <tbody>
            {scenario && anywhere && isLocationUsed(anywhere) && (
              <tr className={selected.includes(ANYWHERE_INDEX) ? "selected" : ""} onClick={(e) => tools.select([ANYWHERE_INDEX], e.shiftKey)} onDoubleClick={() => goTo(ANYWHERE_INDEX)}>
                {cell(ANYWHERE_INDEX)}
                <td><span className="row" style={{ gap: 4 }}><Lock size={10} className="faint" />{locationName(scenario, ANYWHERE_INDEX)}{!isAnywhereIntact(scenario) && <span className="badge warn">{t("off map")}</span>}</span></td>
            ...
            {rows.map((l) => (
              <tr key={l.index} className={selected.includes(l.index) ? "selected" : ""} onClick={(e) => tools.select([l.index], e.shiftKey)} onDoubleClick={() => goTo(l.index)}>
                {cell(l.index)}
                <td>{l.name}{l.inverted && <span className="faint"> {t("· inverted")}</span>}</td>
            ...
            {locations.length === 0 && <tr><td colSpan={8} className="hint">{t("No locations yet — drag on empty ground on the Locations layer to create one.")}</td></tr>}
          </tbody>
        </table>
      </div>
    </DialogFrame>
