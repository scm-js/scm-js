...
function explain(source: AssetSource | null): string {
  if (!source) return t("Still looking.");
  switch (source.kind) {
    case "bundled": return source.desktop ? t("Extracted into the app's data folder from your StarCraft installation.") : t("The graphics ship with this build.");
    case "stored": return t("Terrain and units draw from this copy, and it is here next time.");
    case "none": return t("The editor is running without Blizzard’s assets — terrain is flat colours and units are markers.");
  }
}
...
export function GameDataDialog({ entry }: DialogProps) {
  ...
  const adopted = (next: AssetSource, text: string, switched = !sameFiles(source, next)) => {
    ...
    setMessage({ text });
    toast({ kind: "ok", title: t("Game data ready"), detail: text });
  ...
  const run = async (work: () => Promise<void>) => {
    if (busy) return;
    setMessage(null);
    setBusy({ fraction: 0, label: t("Starting") });
    try {
      await work();
    } catch (err) {
  ...
  const kept = (where: "opfs" | "memory") => (where === "memory" ? " " + t("{Here} keeps no site data, so the copy lasts until the tab closes.", { Here: hostTerms().Here }) : "");
  ...
  const fromBlizzard = () => run(async () => {
    installed(await installFromZipUrl(BLIZZARD_ZIP_URL, progress), t("Blizzard's StarEdit download"));
  });
  ...
  const choose = (id: string) => run(async () => {
    ...
    setMessage({ text: next.profile.id === id ? t("Now drawing from {name}.", { name }) : t("{name} has no copy here any more; drawing from the game's own data.", { name }) });
  });
  const removeSet = (p: GameDataProfile) => run(async () => {
    await removeDataSet(store, p.id);
    setMessage({ text: t("Removed {name}.", { name: p.name }) });
  });
  ...
  return (
    <DialogFrame
      ...
      size={have ? "md" : "lg"}
      footer={<Button variant={have ? "primary" : "default"} onClick={() => close(entry.key)}>{have ? t("Close") : t("Continue without graphics")}</Button>}
    >
      <div className="stack">
        {have ? (
          <Group title={t("Now")}>
            <div className="row" style={{ alignItems: "baseline" }}>
              <span className="grow">{source?.label ?? t("Locating…")}</span>
              {removable && <Button size="sm" variant="danger" disabled={!!busy} onClick={remove}><Trash2 size={11} /> {" "}{t("Remove copy")}</Button>}
            </div>
            <p className="hint" style={{ marginTop: 4 }}>{explain(source)}</p>
            {source && !isDefaultProfile(source.profile.id) && <p className="hint" style={{ marginTop: 4 }}>{t("Data set:")}{" "}<strong>{source.profile.name}</strong>{t(". Units, weapons, upgrades and technologies it renamed show its names.")}</p>}
            {revision > 0 && <p className="hint" style={{ marginTop: 4 }}>{t("Open maps pick the graphics up as they arrive.")}</p>}
          </Group>
        ) : (
          <div className="gd-alert">
            <AlertTriangle size={20} />
            <div>
              <strong>{source === null ? t("Looking for StarCraft’s graphics…") : t("The editor has no StarCraft graphics")}</strong>
              <p>
                {t("Terrain is drawn as flat colours and units as coloured markers, so a map cannot really be seen or edited by eye. The graphics come from StarCraft’s own archives, which cannot ship with the editor — install them once, below, and everything draws properly.")}
              </p>
            </div>
          </div>
        )}
              ...
              <div className="col" style={{ gap: 4 }}>
                {sets.map((p) => (
                  <div key={p.id} className="row" style={{ alignItems: "center", gap: 8 }}>
                    ...
                    <span className="grow" />
                    {!isDefaultProfile(p.id) && <Button size="sm" variant="danger" disabled={!!busy} onClick={() => removeSet(p)}><Trash2 size={11} /> {" "}{t("Remove")}</Button>}
                  </div>
                ))}
              </div>
            ) : (
              <p className="hint">{t("The game's own files. A mod that replaces them in the same formats can be installed beside them and switched to here.")}</p>
            )}
            {adding ? (
              <div className="col" style={{ gap: 6, marginTop: 6 }}>
                <div className="row" style={{ gap: 6, alignItems: "center" }}>
                  <TextInput placeholder={t("Name of the data set (the mod's name)")} value={newName} onChange={(e) => setNewName(e.target.value)} disabled={!!busy} style={{ flex: 1 }} />
                  <Button size="sm" disabled={!!busy || !profileIdFrom(newName)} onClick={() => setFolderInput.current?.click()}><FolderOpen size={11} /> {" "}{t("Choose the folder…")}</Button>
                  <Button size="sm" disabled={!!busy} onClick={() => { setAdding(false); setNewName(""); }}>{t("Cancel")}</Button>
                </div>
                ...
                <p className="hint">
                  {t("A folder holding the mod's files —")}{" "}<span className="mono">arr</span>, <span className="mono">{t("unit")}</span>, <span className="mono">{t("tileset")}</span> {" "}{t("and the rest as loose files or as")}{" "}<span className="mono">{t(".mpq")}</span> {" "}{t("archives — together with")}{" "}<span className="mono">{ARCHIVE_NAMES[0]}</span> and <span className="mono">{ARCHIVE_NAMES[1]}</span>{t(", which the mod's files are laid over. Anything else in the folder is left alone.")}
                </p>
              </div>
            ) : (
          ...
          <>
            <section className="gd-card">
              <div className="gd-card-head">
                <Download size={16} />
                <span className="gd-card-title grow">{t("Download from Blizzard")}</span>
                <span className="badge gold">{t("Recommended")}</span>
              </div>
              <p>
                {t("Blizzard offers the StarCraft map editor as a free download, and it carries the two archives the graphics come from. You do not need StarCraft installed, or a copy of the game at all. The files are extracted here and kept in {here}, so this happens once.", { here: hostTerms().here })}
              </p>
              <Button className="gd-cta" variant="primary" disabled={!!busy} onClick={fromBlizzard}>
                <Download size={14} /> {" "}{t("Download the graphics — 82 MB")}
              </Button>
            </section>
            ...
            <section className="gd-alt">
              ...
              <div className="row" style={{ gap: 6, flexWrap: "wrap" }}>
                ...
                {!desktop && <Button size="sm" disabled={!!busy} onClick={() => fileInput.current?.click()}><FolderOpen size={11} /> {" "}{t("Choose StarDat.mpq and BrooDat.mpq…")}</Button>}
              ...
              {desktop && desktopCopy?.status === "ready" && (
                <p className="hint" style={{ marginTop: 4 }}>{t("App copy: {files} files from {from}, {toLocaleDateString}", { files: desktopCopy.files, from: desktopCopy.from, toLocaleDateString: new Date(desktopCopy.at).toLocaleDateString() })}</p>
              )}
              {desktop && searchDirs.length > 0 && (
                <details className="hint" style={{ marginTop: 4 }}>
                  <summary>{t("Where the search looks")}</summary>
            ...
            {source?.kind === "none" && source.tried.length > 0 && (
              <details className="hint">
                <summary>{t("What was tried")}</summary>
