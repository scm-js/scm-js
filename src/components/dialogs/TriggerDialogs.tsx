/**
 * The two trigger editors over `scenario.triggers` / `scenario.briefing`: Classic
 * (StarEdit-style forms) and Mission Briefing share `TriggerListEditor`, a three-pane
 * list ▸ trigger ▸ condition/action editor whose argument widgets are driven entirely by
 * `data/triggerDefs.ts`. The third, the Text Trigger Editor (TrigEdit syntax), is the
 * TrigEdit plugin (`github.com/scm-js/plugin-trigedit`) over `api.triggers.text`.
 *
...
import { formatAction, formatCondition, summarizeTrigger, triggerComment, withComment, type TriggerNames } from "../../formats/triggers/text";
...
function ArgWidget({ kind, value, onChange, names, scenario }: ArgProps) {
  ...
  switch (kind) {
    ...
    // WAV argument is a path in the archive and takes none.
    case "text":
      return (
        <ColorTextField
          wrapStyle={{ width: 360 }}
...
function ItemList<R extends ConditionRecord | ActionRecord>({ items, setItems, kind, briefing, names, scenario }: {
  ...
  // Escaped: a text argument may carry colour codes, and a raw control byte in a summary
  // row draws as a tofu box. The text format itself keeps the bytes — this is display.
  const line = (r: R) => escapeControls(kind === "condition" ? formatCondition(r as ConditionRecord, names) : formatAction(r as ActionRecord, names, briefing));
