...
import { teamColorLut } from "./teamColor";
...
function teamLut(assets: UnitAssets, colorRow: number): Uint8Array {
  let lut = luts.get(colorRow);
  if (!lut) {
    lut = teamColorLut(assets.teamColors, colorRow);
    luts.set(colorRow, lut);
  }
  return lut;
}
...
/**
 * Frame `frame` of image `imageId` in team colour `colorRow`, drawn through `palette`
 ...
 * worth asking again.
 */
export function getImageFrame(assets: UnitAssets, imageId: number, frame: number, flip: boolean, colorRow: number, palette: Uint8Array, paletteKey: string): ImageFrame | null {
  ...
  const key = `${imageId}:${frame}:${flip ? 1 : 0}:${teamColored ? colorRow : "-"}:${paletteKey}`;
  ...
  drawGrpFrame(grp, Math.min(frame, grp.frames.length - 1), pixels.data, width, 0, 0, drawPalette, teamColored ? teamLut(assets, colorRow) : null, flip);
  if (shadow) {
...
export function getUnitSprite(assets: UnitAssets, unitId: number, colorRow: number, palette: Uint8Array, paletteKey: string): UnitSprite | null {
  ...
  return getImageFrame(assets, imageId, frame, flip, colorRow, palette, paletteKey);
}
